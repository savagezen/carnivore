...
raw_data <- read.delim("raw_data-fat_sugar.txt")

# convert data to numerical matrix
df <- as.matrix(as.data.frame(lapply(raw_data, as.numeric)))

# output file - descriptive stats
sink("descriptive_stats-fat_sugar.txt", split=TRUE, append=FALSE)

# descriptive stats
summary(raw_data)

# output file - correlation and regression
sink("correlation-fat_sugar.txt", split=TRUE, append=FALSE)
